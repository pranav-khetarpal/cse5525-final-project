...
# def get_bert_embedding(text, model, tokenizer, max_length=128):
#     inputs = tokenizer(
#         text,
#         padding="max_length",
#         truncation=True,
...
#     # Extract the [CLS] token embedding (first token's embedding)
...
# # get the cleaned tweet column
# tweets = df["cleaned_tweet"].tolist()

# # load base BERT model and tokenizer from the bert_model folder
# save_directory = "./bert_model"
# # tokenizer = BertTokenizer.from_pretrained(save_directory)
# # model = BertModel.from_pretrained(save_directory)

# tokenizer = BertTokenizer.from_pretrained(save_directory)
# model = BertForSequenceClassification.from_pretrained(save_directory, num_labels=2)  # num_labels=2 for binary sentiment classification

# # set model to evaluation mode
# model.eval()

# # Example tweet
# tweet = "The stock market is looking very strong today!"
# inputs = preprocess_text(tweet, tokenizer)
